...
struct MyCustomKernelWithOptionalInput {
  MyCustomKernelWithOptionalInput(const OrtKernelInfo* info) {
    ...
    EXPECT_TRUE(k_info.HasConfigEntry("session.inter_op.allow_spinning"));
    EXPECT_TRUE(k_info.HasConfigEntry("session.intra_op.allow_spinning"));

    EXPECT_EQ("0", k_info.GetConfigEntry("session.inter_op.allow_spinning"));
    EXPECT_EQ("0", k_info.GetConfigEntry("session.intra_op.allow_spinning"));
  }
